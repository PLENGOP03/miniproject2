...
router.get('/receipt', function (req, res) {
    ...
    const query = `SELECT * 
    FROM orders
    RIGHT JOIN product ON orders.ProductID = product.product_ID
    ...
    mysql.query(query, (err, results, fields) => {
      if (err) {
        ...
        results.forEach((order, index) => {
          ...
          doc.text(`ProductName: ${order.product_Name}`, 100, yPos + 20);
          doc.text(`Quantity: ${order.Quantity}`, 100, yPos + 40);
          doc.text(`Price: ${order.product_price}`, 100, yPos + 60);
