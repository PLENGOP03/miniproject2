...
router.get('/productForm',(req,res)=>{
    ...
    mysql.query(sql,(err, result)=>{
      if(err){
        ...
        res.render('productForm', { product:result });
      }
    })
  });
...
router.post('/updateForm/:id',(req,response)=>{
  var sql = 'UPDATE product SET? WHERE product_ID =?';
...
// ค้นหาข้อมูลตารางรายการสินค้า (search table)

router.get('/searchForm',(req,res)=>{
  console.log(req);
  var sname=req.query.q;
  var sql = "SELECT * FROM product WHERE product_Name LIKE '%"+sname+"%'";
  mysql.query(sql,(err, result)=>{
...
router.post('/memberForm',(req, res)=>{
  ...
  mysql.query(sql,data,(err,result)=>{
    ...
    res.redirect('/addMemSuccess'); //productForm
  }
  })
});
...
router.get('/orders', (req, res) => {
  ...
  mysql.query(sql, (err, result) => {
    if (err) {
      ...
      res.render('orders', { product: result });
    }
  })
});
...
router.get('/finishedOrders', (req, res) => {
 ...
 const sql = `
 SELECT o.*, p.product_Name, p.product_ID, p.product_price,p.image
 FROM orders o
  ...
  mysql.query(sql,(err, result) => {
    if (err) {
      res.status(500).send(err);
       ...
       res.render('finishedOrders', { product: result });
...
router.get('/payment', (req, res) => {
  ...
  mysql.query(sql, (err, result) => {
    if (err) {
      ...
      res.render('payment', { product: result });
    }
  })
});
